...
class TeacherNotificationsRepository {
    ...
    suspend fun getNotifications(): Result<List<NotificationDto>> {
        return try {
            ...
            Log.e("TeacherNotifRepo", "Erro ao carregar notificações", exception)
    ...
    suspend fun markAsRead(notificationId: String): Result<Unit> {
        return try {
            ...
            Log.e("TeacherNotifRepo", "Erro ao marcar como lida", exception)
            Result.failure(exception)
        }
    }
    ...
    suspend fun markAllAsRead(): Result<Unit> {
        return try {
            ...
            Log.e("TeacherNotifRepo", "Erro ao marcar todas como lidas", exception)
            Result.failure(exception)
        }
    }
    ...
    fun subscribeToNotifications(
        ...
        if (user == null) {
            Log.w("TeacherNotifRepo", "Sem utilizador autenticado. Realtime não iniciado.")
            return
        }
        ...
        val channel = supabase.realtime.channel(
            "teacher_notifications_${userId}_${System.currentTimeMillis()}"
        )
        ...
        subscriptionJob = changeFlow
            .onEach {
                Log.d("TeacherNotifRepo", "Nova notificação recebida")
                onNotificationReceived()
            }
            .catch { exception ->
                Log.e("TeacherNotifRepo", "Erro no realtime de notificações", exception)
            }
            .launchIn(scope)
        ...
        scope.launch {
            runCatching {
                ...
                Log.e("TeacherNotifRepo", "Erro ao subscrever canal de notificações", exception)
            }
        }
    }
        ...
        if (channel != null) {
            CoroutineScope(Dispatchers.IO).launch {
                runCatching {
                    ...
                    Log.e("TeacherNotifRepo", "Erro ao cancelar canal de notificações", exception)
                }
            }
        }
    }
